// Gregory Giovannini
// Problem 1 Question 3 - Variation on the Problem
// search_variation.c
...
int search(int *arr, int size, int xThreads, int key, int *foundIndexes)
{
	...
	for (i = 0; i < xThreads; i++)
	{
		// Do not spin up any more threads if all elements have already been found.
		if (globalFoundCount == 3)
		{
			break;
		}
	...
	for (i = 0; i < spawnedThreadCount; i++)
	{
...
void* atomicSearch(void *threadArgs)
{
	...
	{
		// Stop searching immediately if all elements have already been found.
		if (globalFoundCount == 3)
		{
			break;
		}
